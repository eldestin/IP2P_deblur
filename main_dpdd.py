...
from utils.ip2p_ds import IP2P_dataset, DPDD_prompt
...
class Instruct_pix2pix(pl.LightningModule):
    ...
    def __init__(self, params):
        ...
        # used pretrained
        self.noise_scheduler = DDPMScheduler.from_pretrained(self.params.Model.checkpoint, subfolder="scheduler")
    ...
    def image_process(self, img):
        ...
        for i in range(img.shape[0]):
            ...
            img_ls.append(image)
        return img_ls
    # def batch_process_img(self, image):
    #     image = (image / 2 + 0.5).clamp(0, 1).squeeze()
    #     image = (image.permute(1, 2, 0) * 255).to(torch.uint8).cpu().numpy()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        psnr = PSNR(edited_image.float(), gt.float())
        ssim = SSIM(edited_image.float(), gt.float())
        ...
        return {"LPIPS_score":lpips_score, "Clip_image_sim":cos_sim, "MaE":mae, "PSNR":psnr, "SSIM":ssim}
    ...
    def train_dataloader(self):
        ...
        train_set = DPDD_prompt(self.train_datasets,train=True,size=self.crop_size, tokenizer=self.tokenizer)
        train_loader = DataLoader(train_set, batch_size=self.train_batchsize, shuffle=True, num_workers=self.num_workers)

        return train_loader
    
    def val_dataloader(self):
        val_set = DPDD_prompt(self.validation_datasets,train=False,size=256,crop=True, tokenizer=self.tokenizer)
...
if __name__=="__main__":
    ...
    config_path = r'/hpc2hdd/home/hfeng108/multimodal/instructpix2pix/diffusers/training_ip2p/option/IP2P_edited_dpdd.yaml'
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor='LPIPS_score',
        filename='epoch{epoch:02d}-PSNR-{PSNR:.3f}-SSIM-{SSIM:.4f}-LPIPS-{LPIPS_score:.4f}',
    ...
    output_dir = '/hpc2hdd/home/hfeng108/multimodal/instructpix2pix/diffusers/training_ip2p/lgs/IP2P_edited_dpdd'
