...
def batch_PSNR(img, imclean, border=0, ycbcr=True):
    img = img.cpu()
    imclean = imclean.cpu()
    if ycbcr:
...
def batch_SSIM(img, imclean, border=0, ycbcr=True):
    img = img.cpu()
    imclean = imclean.cpu()
    if ycbcr:
        img = rgb2ycbcrTorch(img, True)
...
def PSNR(pred, gt):
    ...
    if rmse == 0:
        return 100
    return torch.tensor(20 * math.log10( 1.0 / rmse))

def PSNR_DPDD(pred, gt, PIXEL_MAX = 1.0):
    imdff = pred - gt
    rmse = torch.mean(imdff**2)
    return 10 * torch.log10(PIXEL_MAX/rmse)
